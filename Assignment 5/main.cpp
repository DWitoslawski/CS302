...
int main() {
    ...
    hashMap<std::string, std::string> keyWordMap; // Stores key-value pairs of <keyword, category>
    ...
    hashMap<std::string, int> accessMap, categoryMap;
    ...
    for (int i = 0; i < numCases; i++)
    {
        ...
        for (auto j : listOfCategories)
        {
            listOfCategories.pop_back();
            keyWordNums.pop_back();
        }

        // Reset hashmaps to default for next case

        hashMap<std::string, std::string> temp;
        hashMap<std::string, int> tmp;

        keyWordMap = temp;
        accessMap = categoryMap = tmp;
